...
(function($){var methods={init:function(options){var defaults={button_width:45,column:0,position:'left',trigger:'mouseenter',};var settings=$.extend(defaults,options);if($('#tablemenu_context_menu').length!=0){alert('Warning! You cannot have multiple instances of tablemenu_context_menu.');}else{$("body").append('<div id="tablemenu_context_menu"></div>');}
$('#tablemenu_context_menu').mouseleave(function(){$('#tablemenu_context_menu').hide();});return this.each(function(e){var o=options;var obj=$(this);var items=$("tbody > tr",obj);items.mouseleave(function(e){if(e.relatedTarget!=null&&e.relatedTarget.id!="tablemenu_context_menu"&&e.relatedTarget.parentNode.id!="tablemenu_context_menu"){$('#tablemenu_context_menu').hide();}});items.bind(settings.trigger,function(e){var menu_items=settings.menu_items;var control_width;var el_data=eval('(['+$(this).attr('el_data')+'])')[0];var el_options=eval('(['+$(this).attr('el_options')+'])')[0];if(settings.width!=null){control_width=settings.width;}else{control_width=Object.keys(el_options).length*settings.button_width;}
$('#tablemenu_context_menu').html('');$.each(menu_items,function(key,val){if(el_options[key]!=null){var button=document.createElement('button');$(button).html('<i class="'+val.icon+'"></i>');if(val.title!=null){$(button).attr('title',val.title);}
if(el_options[key]==0){$(button).attr('disabled','disabled');}
